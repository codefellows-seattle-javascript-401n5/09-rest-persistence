...
storage.getAll = () => {
  return new Promise( (resolve,reject) => {
    ...
    fs.readdir(dataDirectory, (err,files) => {
      if(err) { reject(err); }
      let promises = [];
      // files = ['1008.json', '1009.json', '1010.json']
      ...
      Promise.all(promises)
        .then(contents => {
          ...
          /**
            arr = [
              { id: 1, name: richard, date: 12},
              { id: 2, name: richard, date: 12},
              { id: 3, name: richard, date: 12},
            ]
            for(let obj of arr){
              if obj.id == 2
            }
            arrObj = {
              1 : { id: 1, name: richard, date: 12},
              2 : { id: 2, name: richard, date: 12},
              3 : { id: 3, name: richard, date: 12},
            }
            arrObj[3]
